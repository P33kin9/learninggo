...
func (lex *lexer) consume(want rune) {
	if lex.token != want { // NOTE: Not an example of good error handing.
...
//The read function is a decoder for a small subset fo well-formed
...
// - that the input does no tcantain dotted lists such as (1 2 . 3).
// - that the input does not contain Lisp reader macros such 'x and #'x.
//
// The reflection logic assume
// - that v is always a variable of the approriate type for the
...
// - that if v is a numberic variable, it is a signed integer.
...
func read(lex *lexer, v reflect.Value) {
	...
	case scanner.String:
		s, _ := strconv.Unquote(lex.text()) // NOTE: igoring errors
		v.SetString(s)
		lex.next()
		return
	case scanner.Int:
